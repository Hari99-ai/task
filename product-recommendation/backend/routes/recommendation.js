...
function fallbackRecommendations(query) {
  return [...products]
    .filter((product) => {
      const budget = parseBudget(query);
...
async function getOpenRouterRecommendations(query) {
...
router.post("/recommend", async (req, res) => {
  try {
    ...
    try {
      recommendedProducts = await getOpenRouterRecommendations(trimmedQuery);
    } catch (error) {
      console.warn("AI recommendation failed, using fallback:", error.message);
    }

    if (!recommendedProducts || recommendedProducts.length === 0) {
      recommendedProducts = fallbackRecommendations(trimmedQuery);
    }
